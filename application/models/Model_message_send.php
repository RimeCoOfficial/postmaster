...
class Model_message_send extends CI_Model
{
  ...
  {
    // $this->db->select($this->message_send_table.'.*');
    $this->db->select($this->message_send_table.'.request_id');
    $this->db->select($this->message_send_table.'.verify_id');
    $this->db->select($this->message_send_table.'.subject');
    $this->db->select($this->message_send_table.'.to_email');
    ...
    $this->db->limit($count);
    $this->db->order_by($this->message_send_table.'.request_id', 'DESC');

    // $this->db->where('sent', '1000-01-01 00:00:00');

    $this->db->join($this->message_request_table, $this->message_request_table.'.request_id = '.$this->message_send_table.'.request_id');
